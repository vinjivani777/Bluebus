        ...
        <div class="container-fluid">
                    ...
                    <div class="page-title-box">
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                ...
                                <li class="breadcrumb-item active">Drop Point Details</li>
                            </ol>
                        </div>
                        <h4 class="page-title">Drop Point List</h4>
                    </div>
                </div>
            </div>
            <!-- end page title -->
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <a href="{{ route('vendor.drop-point.add') }}" class="btn btn-primary mb-2" ><i class="fas fa-plus mr-1"></i> Add New Drop Point</a>
                            <table id="basic-datatable" class="table dt-responsive table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$drop->address}}</td>
                                        <td>
                                            <button class="btn status-change {{$drop->status == 1?"btn-outline-primary":"btn-outline-danger"}} btn-rounded waves-effect waves-light btn-sm" value="{{$drop->status==1?"Active":"Disable"}}" id="{{$drop->id}}">{{$drop->status==1?"Active":"Disable"}}</button>
                                        </td>
                                        <td>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myLargeModalLabel">Drop Point view</h4>
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-12 col-md-12 ">
                                            <h4 class="card-title mt-0 mb-0">Drop Point Details</h4>
                                        </div>
...
<script>
    //delete drop point
    $('.remove_drop_point').click(function(){
        ...
        swal({
            ...
            if (result.value) {
                    $.ajax({
                            url:'{{route('vendor.drop-point.destroy')}}',
                        data:{
                            id : c_id
                        },
                        type:'get',
                        success:function(response)
                        {
                            ...
                            swal({
                                title: "Deleted !",
                                text: "Successfull deleted drop point.",
                                type: "success",
                                timer: 500,
    ...
    // Status Change the bus drop point //
    $('.status-change').click(function(){
        var status= $(this).val()=="Active"?0:1;
        var id=$(this).prop('id');
        swal({
                title: "Are you sure?",
                text: "Update the status on bus drop point.",
                type: "warning",
                showCancelButton: !0,
                confirmButtonClass: "btn btn-confirm mt-2",
                cancelButtonClass: "btn btn-cancel ml-2 mt-2",
                ...
                if (result.value) {
                    ...
                    data:{'status':status,'id':id,'model':'DropPoint',"_token": "{{ csrf_token() }}"},
                    success:function(response){
